...
unsplash::Unsplash_Wei_old::Unsplash_Wei_old()
{
...
//unsplash::Unsplash_Wei_old::Unsplash_Wei_old(float interval, std::string saveLoc, int wallpaperW, int wallpaperH) :Unsplash_Wei_old()
//{
//}

unsplash::Unsplash_Wei_old::Unsplash_Wei_old(float interval, std::string saveLoc, int wallpaperW, int wallpaperH)
{
	//initialize the temp image file 
...
void unsplash::Unsplash_Wei_old::setRes(const int width, const int height)
{
	if (!(width*height))
	{
...
void unsplash::Unsplash_Wei_old::setDefaultSaveLoc()
{
	char picFolder[256];
...
int unsplash::Unsplash_Wei_old::setWallpaper()
{
...
void unsplash::Unsplash_Wei_old::loopExec(bool enableKeystroke)
{
	float refreshMsec, retryMsec, waitMsec;
	if (enableKeystroke)
	{
		std::thread keyStrokeDetectThread(&unsplash::Unsplash_Wei_old::enableKeystrokeCombo, this);
		while (true)
		{
			...
			if (!keyStrokeDetectThread.joinable())
				keyStrokeDetectThread = std::thread(&unsplash::Unsplash_Wei_old::enableKeystrokeCombo, this);
...
void unsplash::Unsplash_Wei_old::enableKeystrokeCombo()
{
	//std::thread loopExec_thread(&unsplash::Unsplash_Wei_old::loopExec,this); //http://stackoverflow.com/questions/10998780/stdthread-calling-method-of-class
	while (true)
	{
		...
		else if ((GetAsyncKeyState(VK_CONTROL) & 0x8000) && (GetAsyncKeyState(VK_MENU) & 0x8000) && (GetAsyncKeyState(0x4E) & 0x8000)) // manually refresh the wallpaper
		{
			...
			manualRefresh = false;
			loopExec_thread=std::thread(&unsplash::Unsplash_Wei_old::loopExec, this);*/
		}
		Sleep(minMsec);
	}	
}